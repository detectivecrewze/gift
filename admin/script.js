...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Recover secret
    if (localStorage.getItem('arcade_admin_secret') && adminSecretInput) {
        adminSecretInput.value = localStorage.getItem('arcade_admin_secret');
    }
    ...
    const fetchGifts = async () => {
        ...
        localStorage.setItem('arcade_admin_secret', secret);
    ...
    const renderTable = (gifts) => {
        ...
        tableBody.innerHTML = gifts.map(gift => {
            ...
            // Because this is Arcade, the URL is root /id
            ...
            // Format Apps active
            const apps = gift.activeApps || {};
            let appCount = 0;
            if(apps.moments) appCount++;
            if(apps.journey) appCount++;
            if(apps.quiz) appCount++;
            if(apps.bucketList) appCount++;

            return `
                <tr class="${isSelected ? 'bg-brand-cream/5' : ''}">
                    <td class="p-5 border-b border-brand-gold/5 text-center">
                        <input type="checkbox" data-id="${gift.giftId}" ${isSelected ? 'checked' : ''} class="custom-checkbox row-checkbox">
                    </td>
                    <td class="p-5 border-b border-brand-gold/5">
                        <div class="flex flex-col gap-1">
                            <span class="text-xs font-mono font-bold text-brand-cream tracking-tight">${gift.giftId}</span>
                    ...
                    <td class="p-5 border-b border-brand-gold/5">
                        <div class="flex flex-col gap-1">
                            ...
                            <span class="text-[9px] uppercase tracking-widest text-brand-cream/60 font-medium">🎵 ${gift.ambient}</span>
                        </div>
                    </td>
                    <td class="p-5 border-b border-brand-gold/5">
    ...
    const applyFilters = () => {
        ...
        const filtered = allGiftsRaw.filter(g => {
            const matchesSearch = g.giftId.toLowerCase().includes(query) || (g.recipientName || '').toLowerCase().includes(query);
    ...
    selectAllCheckbox.addEventListener('change', e => {
        if(e.target.checked) allGifts.forEach(g => selectedIds.add(g.giftId));
